...
class GPT(nn.Module):
    ...
    def configure_optimizer(self, weight_decay, learning_rate, device):
        ...
        print(
            f"num decayed param tensors: {len(decay_params)} -> {num_decay_params:,} parameters"
        )
        print(
            f"num non-decayed param tensors: {len(nodecay_params)} -> {num_nodecay_params:,} parameters"
        )
        ...
        optimizer = torch.optim.AdamW(
            optim_groups, lr=learning_rate, betas=(0.9, 0.5), eps=1e-8, fused=use_fused
        )
...
class DataLoaderLite:
    def __init__(self, B, T, split) -> None:
        ...
        data_root = os.path.join(os.path.dirname(__file__), "..", "fineweb", "edu_fineweb10B")
